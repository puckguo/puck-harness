...
/rewind               回退到之前任一节点（双击 Esc 同样触发）：对话 / 代码 / 两者
...
- **@文件引用**：输入 `@` 立即索引当前项目（含子目录，异步流式加载、缓存 30 秒），按光标后的名称片段模糊过滤（文件名精确 > 文件名模糊 > 路径匹配，浅层优先），↑/↓ 选择、Tab/Enter 插入路径、Esc 关闭；提交时自动把 @ 提及的文件内容附加到消息里（每个 ≤ 64KB、最多 8 个，模型不用再 read 一遍）；一次性模式 `puck "看看 @package.json"` 同样生效。
...
- **双击 Esc 回退（/rewind）**：Claude Code 同款——空闲时连按两次 Esc（或运行中按：第一下停止本轮，第二下请求回退，结算后自动弹出）打开回退选择器：每轮对话发出前都会自动记录一个**节点**（transcript 快照 + 会话日志位置 + 该轮 write/edit 改动文件的首次前快照，存在 `.puck/checkpoints/<sessionId>/`，恢复后可继续回退到更早节点，/resume 后仍可用）。选中任一节点后可选择恢复「对话 + 代码 / 仅对话 / 仅代码」：对话 = 上下文截断到该消息之前（会话日志追加 `rewind` 标记，重放自动截断，被丢弃的部分仍在磁盘上）；代码 = 把该节点之后改动的文件恢复到当时的字节内容（该轮新建的文件会被删除）。限制：bash 命令造成的文件改动、以及超过 32MB 的文件不在快照范围内（回退时提示无法恢复）。
...
- **/resume 默认只列 10 条，按 `m` 加载更多**：合并列表（跨项目 + 外部 harness）动辄 50+ 行，一次全量展示会淹没用户最想要的近期会话。标题带 `已显 x/y` 计数；`m` 每次多显 10 条（管道模式输入 `m` 回车同样生效），已展开的条数在 `a`/`c` 切换后保留。`r` 重新扫描（本地 · 全局注册表 · 外部 harness）并回到首页。
- **全部目录视图包含其它项目的 puck 会话**：puck 的会话文件存在各项目自己的 `.puck/sessions/`，单靠当前目录扫描不到；/resume 通过两个全局来源发现它们——**全局注册表 `~/.puck/sessions.json`（主）**：session 层在每次创建/加载会话时自动登记文件的绝对路径（原子写、自剪枝死条目、上限 2000 条，`PUCK_SESSION_REGISTRY=0` 可关闭），因此 `--no-memory`、无 node:sqlite 时也能发现；**sqlite 索引 `~/.puck/index.db`（辅）**：覆盖注册表出现之前的旧会话。候选逐条校验文件存在后并入列表——当前目录没有 puck 历史时，不会只剩 pi/codex/claude 的外部会话。选中后从**原文件**恢复，后续对话继续追加回原项目（索引行的 project 始终指向文件所在项目，不随恢复位置变）。两个来源均缺失时优雅降级为仅本地 + 外部扫描。
